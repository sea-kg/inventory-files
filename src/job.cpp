...
Job::Job(QSqlDatabase *pDB, QString sStartDir, INotifyService *pNotifyService) {
    m_pDB = pDB;
    m_sStartDir = sStartDir;
    m_nCountFiles = 0;
    m_sState = "begin";
    m_pDetection = new Detection();
	m_pNotifyService = pNotifyService;
}
...
void Job::run() {
    ...
    m_sState = "Scanning...";
	// TODO notify
	m_pNotifyService->showSystemMessage("Scanning...", m_sStartDir);
    m_nCountFiles = 0;
    ...
    m_sState = "Completed";
	m_pNotifyService->showSystemMessage("Scanning completed", m_sStartDir);
}
